...
def seq2seq_pad_concat_convert(xy_batch, device, eos_id=1, bos_id=3):
    ...
    x_block, y_in_block, y_out_block = Variable(torch.LongTensor(x_block), volatile=True), \
                                       Variable(torch.LongTensor(y_in_block), volatile=True), \
                                       Variable(torch.LongTensor(y_out_block), volatile=True)
